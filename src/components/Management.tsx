...
export const Management: React.FC<ManagementProps> = ({ lang, onDataChange, currentUser, onViewProject }) => {
  ...
  const handleDeleteProject = (id: string) => {
      ...
      if(confirm(lang === 'es' ? `¿Estás seguro de eliminar el proyecto "${proj?.name}"? Esta acción no se puede deshacer.` : `Delete project "${proj?.name}"? This cannot be undone.`)) {
          deleteProject(id);
          loadData();
  ...
  const handleDeleteAgent = (id: string) => {
      ...
      if(confirm(lang === 'es' ? `¿Eliminar al agente "${agent?.name}"?` : `Delete agent "${agent?.name}"?`)) {
          deleteAgent(id);
          loadData();
          toast.success(t.removed);
          if(onDataChange) onDataChange();
  ...
  const handleDeleteUser = (id: string) => {
      ...
      if(id === 'admin') {
          toast.error(lang === 'es' ? "No puedes eliminar la cuenta de Administrador principal." : "Cannot delete main Administrator account.");
          return;
      }
      if(confirm(lang === 'es' ? `¿Eliminar al auditor "${user?.name}"?` : `Delete auditor "${user?.name}"?`)) {
          deleteUser(id);
          loadData();
          toast.success(t.removed);
          if(onDataChange) onDataChange();
  ...
  const handleDeleteRubric = (id: string) => {
      if(confirm(lang === 'es' ? "¿Eliminar este indicador de la rúbrica?" : "Delete this rubric indicator?")) {
          deleteRubricItem(id);
          loadData();
          toast.success(t.removed);
          if(onDataChange) onDataChange();
        ...
        {activeTab === 'projects' && (
            <div className="p-10 space-y-12 animate-fade-in-up">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-10 border-t border-slate-100 dark:border-slate-800">
                    {projects.map(p => (
                        <div key={p.id} className="bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-800 p-6 rounded-[2rem] group transition-all hover:border-indigo-500/50 relative overflow-hidden">
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-xl flex items-center justify-center text-indigo-600 shadow-sm"><Briefcase className="w-6 h-6" /></div>
                    ...
                    {rubric.map(item => (
                        <div key={item.id} className={`p-6 rounded-3xl border-2 flex justify-between items-center transition-all ${item.isActive ? 'border-indigo-500 bg-indigo-500/5' : 'border-slate-100 dark:border-slate-800 opacity-60'}`}>
                             ...
                             <div className="flex gap-2">
                                ...
                                <button onClick={() => handleDeleteRubric(item.id)} className="p-3 text-slate-400 hover:text-red-500 rounded-xl"><Trash2 className="w-4 h-4" /></button>
                             </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="p-10 space-y-10 animate-fade-in-up">
                <div className="max-w-3xl bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 mx-auto">
                    ...
                    <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-500 uppercase ml-2">PIN Acceso</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {users.map(u => (
                        <div key={u.id} className="p-6 bg-slate-50 dark:bg-slate-850 rounded-[2rem] border border-slate-100 dark:border-slate-800 flex justify-between items-center group transition-all hover:shadow-lg">
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-white shadow-md ${u.role === UserRole.ADMIN ? 'bg-indigo-600' : 'bg-emerald-500'}`}>
                                    {getInitials(u.name)}
                                </div>
                                <div>
                            ...
                            <div className="flex gap-2">
                                ...
                                {u.id !== 'admin' && <button onClick={() => handleDeleteUser(u.id)} className="p-2 text-slate-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>}
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="p-10 space-y-10 animate-fade-in-up">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {agents.map(a => {
                        ...
                        return (
                            <div key={a.id} className="p-6 bg-slate-50 dark:bg-slate-850 border border-slate-100 dark:border-slate-800 rounded-[2rem] flex flex-col items-center group relative hover:border-indigo-500/50 transition-all hover:shadow-xl">
                                <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                    ...
                                    <button onClick={() => handleDeleteAgent(a.id)} className="p-1.5 text-slate-400 hover:text-red-500 bg-white dark:bg-slate-800 rounded-lg shadow-sm"><Trash2 className="w-3.5 h-3.5" /></button>
                                </div>
                                <div className="w-14 h-14 bg-indigo-600 rounded-2xl flex items-center justify-center text-white font-black text-xl mb-4 group-hover:scale-110 transition-transform shadow-lg shadow-indigo-600/20">
                                    {getInitials(a.name)}
                                </div>
