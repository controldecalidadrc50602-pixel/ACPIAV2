        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-white">
            ...
            <section className="pt-40 pb-20 px-6 relative overflow-hidden">
                <div className="max-w-5xl mx-auto text-center relative z-10">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 font-bold text-xs uppercase tracking-widest mb-8 border border-indigo-100 dark:border-indigo-800 animate-fade-in-up">
                        <Sparkles className="w-4 h-4" /> Nuevo: Auditoría por Voz con IA
                    </div>
                    ...
                    <p className="text-xl md:text-2xl text-slate-500 dark:text-slate-400 mb-12 max-w-3xl mx-auto leading-relaxed">
                        Transforma tus auditorías con <strong className="text-indigo-600 dark:text-indigo-400">Inteligencia Artificial</strong>.
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-center gap-6">
                        <Link to="/login" className="w-full md:w-auto px-10 py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[2rem] font-black uppercase tracking-widest shadow-2xl shadow-indigo-600/40 hover:-translate-y-1 transition-all text-sm flex items-center justify-center gap-3">
                            <Zap className="w-5 h-5" /> Prueba Gratis
                        </Link>
                ...
                <div className="max-w-7xl mx-auto px-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                        <FeatureCard
                            ...
                            desc="Olvídate de escuchar horas de audio. ACPIA transcribe y califica en segundos."
                        />
                        <FeatureCard
                            icon={<Lock className="w-8 h-8 text-emerald-500" />}
                            title="Seguridad ISO 27001"
                            desc="Redacción automática de datos sensibles (PII) y logs de auditoría inmutables."
                        />
                        <FeatureCard
                            icon={<BarChart className="w-8 h-8 text-indigo-500" />}
                            title="Reportes Ejecutivos"
                            desc="Genera PDFs profesionales y dashboards de tendencias para tus clientes."
                        />
                    </div>
                </div>
            </section>
            ...
            <section className="py-20 border-y border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                    <Stat number="100%" label="Cobertura" />
                    <Stat number="99.9%" label="Uptime" />
                    <Stat number="10x" label="Más Rápido" />
                    <Stat number="ISO" label="Compliant" />
                </div>
            </section>
            ...
            <section className="py-24 px-6">
                <div className="max-w-5xl mx-auto">
                    <div className="text-center mb-16">
                        <h2 className="text-4xl font-black mb-4 uppercase tracking-tighter">Planes Flexibles</h2>
                        <p className="text-slate-500 text-lg">Escala con tu operación. Sin contratos forzosos.</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                        <PricingCard title="Free" price="$0" features={['10 Auditorías/mes', 'Análisis Básico', '1 Usuario']} />
                        <PricingCard title="Standard" price="$29" recommended features={['100 Auditorías/mes', 'Reportes PDF', 'Soporte Email', '3 Usuarios']} />
                        <PricingCard title="Pro" price="$99" features={['Ilimitado', 'API Access', 'Marca Blanca', 'Usuarios Ilimitados']} />
                    </div>
                </div>
            </section>
...
const PricingCard = ({ title, price, features, recommended }: any) => (
    <div className={`p-8 rounded-[2.5rem] relative ${recommended ? 'bg-indigo-600 text-white shadow-2xl shadow-indigo-600/40 scale-105 z-10' : 'bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800'}`}>
        {recommended && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-yellow-400 text-indigo-900 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">Más Popular</div>}
        <h3 className={`text-xl font-black uppercase tracking-widest mb-2 ${recommended ? 'text-indigo-200' : 'text-slate-400'}`}>{title}</h3>
        <div className="text-5xl font-black mb-8 tracking-tighter">{price}<span className="text-lg font-bold opacity-50">/mo</span></div>
        ...
        <Link to="/login" className={`w-full block text-center py-4 rounded-xl font-black uppercase tracking-widest text-xs transition-all ${recommended ? 'bg-white text-indigo-600 hover:bg-slate-100' : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
            Elegir {title}
        </Link>
    </div>
);
